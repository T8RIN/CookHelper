...
class FridgeViewModel @Inject constructor(
    ...
    init {
        getFridgeListUseCase().onEach { result ->
            when (result) {
                is Action.Success -> {
                    _fridgeListState.update { FridgeListState(products = result.data?.sortedBy { it.name }) }
                }
                is Action.Error -> {
                    _fridgeListState.update { copy(isLoading = false) }
                    sendEvent(Event.ShowToast(UIText.DynamicString(result.message ?: "")))
                }
                is Action.Loading -> {
                    _fridgeListState.update { FridgeListState(isLoading = true) }
                }
