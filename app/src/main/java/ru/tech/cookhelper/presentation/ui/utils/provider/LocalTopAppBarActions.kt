...
val LocalTopAppBarActions = compositionLocalOf<TopAppBarActionsState> {
    error("TopAppBarActionsState not present")
}

@Composable
operator fun TopAppBarActionsState.invoke(rowScope: RowScope) = this.value?.invoke(rowScope)
